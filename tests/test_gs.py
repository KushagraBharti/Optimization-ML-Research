...
def test_gs_single_tour_finish(tol: float) -> None:
    ...
    p, q = tours[0]
    assert math.isclose(p, segments[0][0], abs_tol=EPS)
    assert math.isclose(q, segments[-1][1], abs_tol=EPS)
...
def test_gs_tie_break_prefers_right(tol: float) -> None:
    ...
    check_tours_feasible(h, L, tours, tol=tol)
    first_tour = tours[0]
    assert first_tour[1] >= segments[-1][1] - EPS
...
def test_gs_infeasible_due_to_farthest_endpoint() -> None:
    ...
    with pytest.raises(ValueError) as exc_info:
        greedy_min_tours_ref(segments, h=h, L=L)
...
def test_gs_left_sweep_progress(tol: float) -> None:
    ...
    remaining = [
        seg
        for seg in segments
        if not (first_tour[0] - EPS <= seg[0] and seg[1] <= first_tour[1] + EPS)
    ]
    if remaining:
        new_left = min(a for a, _ in remaining)
        assert new_left > first_left + EPS
...
if HAVE_HYPOTHESIS:
    ...
    def test_gs_monotonicity_in_L(data, tol: float) -> None:
        segments, h, L = data
        base_count, base_tours = greedy_min_tours_ref(segments, h=h, L=L)
        check_cover_exact(segments, base_tours)
        check_tours_feasible(h, L, base_tours, tol=tol)
        L_scaled = 1.2 * L
        larger_count, larger_tours = greedy_min_tours_ref(segments, h=h, L=L_scaled)
...
def test_gs_scaling_invariance(scale: float, tol: float) -> None:
    ...
    scaled_h = h * scale
    scaled_L = L * scale
    scaled_count, scaled_tours = greedy_min_tours_ref(
...
def test_gs_reflection_invariance(tol: float) -> None:
    ...
    ref_count, ref_tours = greedy_min_tours_ref(reflected_segments, h=h, L=L)
