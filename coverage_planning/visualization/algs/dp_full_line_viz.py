"""Instrumented copy of the full-line DP solver."""
...
def _dp_one_side_tail_core(
    ...
    *,
    ...
    capture_trace: bool,
    tol: float,
) -> Tuple[List[float], List[float], Optional[TailPlan], Optional[Dict[str, Any]]]:
    if not segments:
        base_tail = [0.0]
                ...
                "per_candidate": [
                    {
                        ...
                        "tried_transitions": [],
                        "picked": {"next_idx": None, "cost": 0.0, "case": "tail_terminal"},
                    }
                ],
            }
            if compute_plan:
                trace_payload["plan_next"] = [-1]
                trace_payload["plan_edges"] = [(0.0, 0.0)]
                trace_payload["plan_kind"] = ["terminal"]
        return base_tail, [0.0], plan, trace_payload
    ...
    trace_entries_rev: List[Dict[str, Any]] = [] if capture_trace else []
    ...
    for idx in range(len(candidates) - 1, -1, -1):
        ...
        attempts: List[Dict[str, Any]] = [] if capture_trace else []
        ...
        if kind == "beyond":
            length = tour_length(q, rights[-1], h)
            best_cost = length
            ...
            t = pos_idx
            start_j = seg_idx if seg_idx is not None else t + 1
            if start_j > t:
        ...
        if capture_trace:
            trace_entries_rev.append(
                {
                    "index": idx,
                    "candidate": float(q),
                    "classification": kind,
                    "tried_transitions": attempts,
                    "picked": {
                        "next_idx": None if best_next is None or best_next < 0 else best_next,
                        "cost": float(best_cost),
                        "case": best_kind or "tail",
                        "edge": (float(best_edge[0]), float(best_edge[1])),
                    },
                }
            )
    ...
    trace_payload = None
    if capture_trace:
        trace_entries_rev.reverse()
        trace_payload = {
            "candidates": [float(x) for x in candidates],
            "per_candidate": trace_entries_rev,
        }
        if compute_plan:
            trace_payload["plan_next"] = list(plan_next)
            trace_payload["plan_edges"] = [(float(p), float(q)) for p, q in plan_edge]
            trace_payload["plan_kind"] = list(plan_kind)

    return Tail, candidates, plan, trace_payload
...
def dp_one_side_tail(
    ...
    Tail, candidates, _plan, _trace = _dp_one_side_tail_core(
        segments,
        h,
        L,
        debug=debug,
...
def dp_one_side_tail_with_plan(
    ...
    *,
    ...
    tol: float = TOL_NUM,
    debug: Optional[Dict[str, Any]] = None,
) -> Tuple[List[float], List[float], TailPlan] | Tuple[List[float], List[float], TailPlan, Dict[str, Any]]:
    Tail, candidates, plan, trace_payload = _dp_one_side_tail_core(
        segments,
        h,
        L,
        debug=debug,
    ...
    if trace:
        if trace_payload is None:
            raise RuntimeError("Tail trace capture failed")
        return Tail, candidates, plan, trace_payload
...
def _dp_full_line_core(
    ...
    *,
    ...
    left_trace: Optional[Dict[str, Any]] = None
    right_trace: Optional[Dict[str, Any]] = None
    tail_trace: Optional[Dict[str, Any]] = None
    left_candidates: List[float] = []
    right_candidates: List[float] = []
    tail_candidates: List[float] = []
    bridge_attempts: List[Dict[str, Any]] = [] if capture_trace else []
    ...
    if not segs:
        ...
        if capture_trace:
            trace_payload = {
                ...
                "tail_trace": None,
                "bridge_attempts": [],
                "final_mode": "no_bridge",
            }
        if debug is not None:
            debug.clear()
            debug.update(debug_payload)
        return 0.0, [], debug_payload, None, solutions, trace_payload
    ...
    if not left_ref:
        if compute_plan:
            if capture_trace:
                Sigma_R, C_R, plan_R, right_trace = dpos_with_plan(
                    right, h, L, trace=True, tol=tol, debug=right_debug
                )
            else:
                Sigma_R, C_R, plan_R = dpos_with_plan(right, h, L, trace=False, tol=tol, debug=right_debug)
            right_candidates = list(C_R)
        ...
        if capture_trace:
            trace_payload = {
                "left_candidates": [],
                "right_candidates": [float(x) for x in right_candidates],
                "tail_candidates": [],
                "left_trace": None,
        ...
        if debug is not None:
            debug.clear()
            debug.update(debug_payload)
        return Sigma_R[-1], tours, debug_payload, plan_context, solutions, trace_payload

    if not right:
        if compute_plan:
            if capture_trace:
                Sigma_L, C_L, plan_L, left_trace = dpos_with_plan(
                    left_ref, h, L, trace=True, tol=tol, debug=left_debug
                )
            else:
                Sigma_L, C_L, plan_L = dpos_with_plan(left_ref, h, L, trace=False, tol=tol, debug=left_debug)
            left_candidates = list(C_L)
            plan_context = FullLinePlanContext(
                C_left=tuple(C_L),
                plan_left=plan_L,
                C_right=(),
        ...
        if capture_trace:
            trace_payload = {
                "left_candidates": [float(x) for x in left_candidates],
                "right_candidates": [],
                "tail_candidates": [],
        ...
        if debug is not None:
            debug.clear()
            debug.update(debug_payload)
        return Sigma_L[-1], tours, debug_payload, plan_context, solutions, trace_payload
    ...
    if compute_plan:
        if capture_trace:
            ...
            Tail_R, C_tail_R, plan_tail, tail_trace = dp_one_side_tail_with_plan(
                right, h, L, trace=True, tol=tol, debug=tail_debug
            )
        else:
            Sigma_L, C_L, plan_L = dpos_with_plan(left_ref, h, L, trace=False, tol=tol, debug=left_debug)
            Sigma_R, C_R, plan_R = dpos_with_plan(right, h, L, trace=False, tol=tol, debug=right_debug)
        ...
        for q in C_tail_R:
            ...
            if capture_trace:
                attempt_record = {
                    "P": float(p),
                    "q": float(q),
                    "bridge_len": float(tour_len),
                    "accepted": False,
                }
            if tour_len > L + EPS:
                if capture_trace and attempt_record is not None:
                    attempt_record["reason"] = "exceeds_budget"
                    bridge_attempts.append(attempt_record)
                break
            try:
                p_star = _find_maximal_p_safe(q, h, L)
            except ValueError:
                if capture_trace and attempt_record is not None:
                    attempt_record["reason"] = "maximal_p_failure"
                    bridge_attempts.append(attempt_record)
                continue
            if abs(p - p_star) > tol:
                if capture_trace and attempt_record is not None:
                    attempt_record["reason"] = "p_mismatch"
                    bridge_attempts.append(attempt_record)
                continue
            if abs(tour_len - L) > 1e-6:
                if capture_trace and attempt_record is not None:
                    attempt_record["reason"] = "length_tolerance"
                    bridge_attempts.append(attempt_record)
                continue
            ...
            if capture_trace and attempt_record is not None:
                ...
                attempt_record["total_cost"] = float(total)
                attempt_record["left_cost"] = float(left_cost)
                attempt_record["right_cost"] = float(right_cost)
                attempt_record["selected"] = selected
                bridge_attempts.append(attempt_record)
    ...
    if capture_trace:
        trace_payload = {
            "left_candidates": [float(x) for x in left_candidates],
            "right_candidates": [float(x) for x in right_candidates],
            "tail_candidates": [float(x) for x in tail_candidates],
            "left_trace": left_trace,
            "right_trace": right_trace,
            ...
            "best_bridge_pair": (
                (float(best_bridge_pair[0]), float(best_bridge_pair[1])) if best_bridge_pair is not None else None
            ),
        }
    ...
    return best_cost, best_tours, debug_payload, plan_context, solutions, trace_payload
...
def dp_full_line_ref(
    ...
    cost, _tours, debug_payload, _context, _solutions, _trace = _dp_full_line_core(
        segments,
        h,
        L,
        debug=debug,
...
def dp_full_with_plan(
    ...
    *,
    ...
    tol: float = TOL_NUM,
    debug: Optional[Dict[str, Any]] = None,
) -> Tuple[float, List[Tuple[float, float]], Dict[str, Any]] | Tuple[float, List[Tuple[float, float]], Dict[str, Any], Dict[str, Any]]:
    cost, tours, debug_payload, context, solutions, trace_payload = _dp_full_line_core(
        segments,
        h,
        L,
        debug=debug,
    ...
    meta["solutions"] = sorted(
        solutions,
        key=lambda rec: (
            rec.get("cost", math.inf),
            rec.get("mode", ""),
            rec.get("left_index", -1),
            rec.get("tail_index", -1),
        ),
    )[:16]

    if trace:
        if trace_payload is None:
            raise RuntimeError("Trace capture failed for full-line DP")
        return cost, tours, meta, trace_payload
...
def dp_full_line_with_plan(
    ...
    *,
    ...
    cost, tours, meta = dp_full_with_plan(
        segments,
        h,
        L,
        trace=False,
